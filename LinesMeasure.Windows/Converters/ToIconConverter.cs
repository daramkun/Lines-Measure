...
{
	class ToIconConverter : IValueConverter
	{
		...
		public object Convert ( object value, Type targetType, object parameter, CultureInfo culture )
		{
			if ( value is FileNode )
			{
				...
				IntPtr hIcon = GetFileIcon ( node.FullPath );
		...
		private BitmapSource LoadFileIcon ( string path )
		{
			string key = null;
			...
			IntPtr hIcon = File.GetAttributes ( path ).HasFlag ( FileAttributes.Directory )
				? GetFolderIcon ( path )
				: GetFileIcon ( path );
			var icon = Imaging.CreateBitmapSourceFromHIcon ( hIcon, Int32Rect.Empty, BitmapSizeOptions.FromWidthAndHeight ( 16, 16 ) );
			DestroyIcon ( hIcon );
